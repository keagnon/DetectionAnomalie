# # pipeline.py
# from kedro.pipeline import Pipeline, node
# from .nodes import split_data, train_model, evaluate_model, predict, train_random_forest, save_predictions_to_text, display_results

# def create_pipeline(**kwargs):
#     return Pipeline(
#         [
#             node(
#                 func=split_data,
#                 inputs=["preprocessed_energy_data", "params:test_size", "params:random_state"],
#                 outputs=["X_train", "X_test", "y_train", "y_test"],
#                 name="split_data_node",
#             ),
#             node(
#                 func=train_model,
#                 inputs=["X_train", "y_train"],
#                 outputs="linear_model",
#                 name="train_linear_model_node",
#             ),
#             node(
#                 func=train_random_forest,
#                 inputs=["X_train", "y_train"],
#                 outputs="rf_model",
#                 name="train_rf_model_node",
#             ),
#             node(
#                 func=evaluate_model,
#                 inputs=["linear_model", "X_test", "y_test"],
#                 outputs="linear_rmse",
#                 name="evaluate_linear_model_node",
#             ),
#             node(
#                 func=evaluate_model,
#                 inputs=["rf_model", "X_test", "y_test"],
#                 outputs="rf_rmse",
#                 name="evaluate_rf_model_node",
#             ),
#             node(
#                 func=predict,
#                 inputs=["rf_model", "X_test"],
#                 outputs="rf_predictions",
#                 name="predict_rf_node",
#             ),
#             node(
#                 func=save_predictions_to_text,
#                 inputs=["rf_predictions", "output_predictions"],
#                 outputs=None,
#                 name="save_predictions_node",
#             ),
#             node(
#                 func=display_results,
#                 inputs=["linear_rmse", "rf_rmse", "rf_predictions"],
#                 outputs=None,
#                 name="display_results_node",
#             ),
#         ]
#     )


# from kedro.pipeline import Pipeline, node
# from .nodes import (
#     split_data,
#     train_model,
#     evaluate_model,
#     predict,
#     train_random_forest,
#     train_xgboost,  # Assurez-vous que cette fonction est bien définie dans votre fichier nodes.py
#     save_predictions_to_text,
#     display_results
# )

# def create_pipeline(**kwargs) -> Pipeline:
#     return Pipeline(
#         [
#             node(
#                 func=split_data,
#                 inputs=["preprocessed_energy_data", "params:test_size", "params:random_state"],
#                 outputs=["X_train", "X_test", "y_train", "y_test"],
#                 name="split_data_node",
#             ),
#             node(
#                 func=train_model,
#                 inputs=["X_train", "y_train"],
#                 outputs="linear_model",
#                 name="train_linear_model_node",
#             ),
#             node(
#                 func=evaluate_model,
#                 inputs=["linear_model", "X_test", "y_test"],
#                 outputs=["linear_rmse", "linear_r2"],
#                 name="evaluate_linear_model_node",
#             ),
#             node(
#                 func=train_random_forest,
#                 inputs=["X_train", "y_train"],
#                 outputs="rf_model",
#                 name="train_rf_model_node",
#             ),
#             node(
#                 func=evaluate_model,
#                 inputs=["rf_model", "X_test", "y_test"],
#                 outputs=["rf_rmse", "rf_r2"],
#                 name="evaluate_rf_model_node",
#             ),
#             node(
#                 func=train_xgboost,
#                 inputs=["X_train", "y_train"],
#                 outputs="xgb_model",
#                 name="train_xgb_model_node",
#             ),
#             node(
#                 func=evaluate_model,
#                 inputs=["xgb_model", "X_test", "y_test"],
#                 outputs=["xgb_rmse", "xgb_r2"],
#                 name="evaluate_xgb_model_node",
#             ),
#             node(
#                 func=predict,
#                 inputs=["xgb_model", "X_test"],
#                 outputs="xgb_predictions",
#                 name="predict_xgb_node",
#             ),
#             node(
#                 func=display_results,
#                 inputs=["linear_rmse", "linear_r2", "rf_rmse", "rf_r2", "xgb_rmse", "xgb_r2", "xgb_predictions"],
#                 outputs=None,
#                 name="display_results_node",
#             ),
#             node(
#                 func=save_predictions_to_text,
#                 inputs=["xgb_predictions", "params:output_path"],
#                 outputs=None,
#                 name="save_predictions_node",
#             ),
#         ]
#     )



from kedro.pipeline import Pipeline, node
from .nodes import (
    split_data,
    train_model,
    evaluate_model,
    predict,
    train_random_forest,
    train_xgboost,
    save_predictions_to_text,
    display_results
)

def create_pipeline(**kwargs) -> Pipeline:
    return Pipeline(
        [
            node(
                func=split_data,
                inputs=["preprocessed_energy_data", "params:test_size", "params:random_state"],
                outputs=["X_train", "X_test", "y_train", "y_test"],
                name="split_data_node",
            ),
            node(
                func=train_model,
                inputs=["X_train", "y_train"],
                outputs="linear_model",
                name="train_linear_model_node",
            ),
            node(
                func=evaluate_model,
                inputs=["linear_model", "X_test", "y_test"],
                outputs=["linear_rmse", "linear_r2"],
                name="evaluate_linear_model_node",
            ),
            node(
                func=train_random_forest,
                inputs=["X_train", "y_train"],
                outputs="rf_model",
                name="train_rf_model_node",
            ),
            node(
                func=evaluate_model,
                inputs=["rf_model", "X_test", "y_test"],
                outputs=["rf_rmse", "rf_r2"],
                name="evaluate_rf_model_node",
            ),
            node(
                func=train_xgboost,
                inputs=["X_train", "y_train"],
                outputs="xgb_model",
                name="train_xgb_model_node",
            ),
            node(
                func=evaluate_model,
                inputs=["xgb_model", "X_test", "y_test"],
                outputs=["xgb_rmse", "xgb_r2"],
                name="evaluate_xgb_model_node",
            ),
            node(
                func=predict,
                inputs=["xgb_model", "X_test"],
                outputs="xgb_predictions",
                name="predict_xgb_node",
            ),
            node(
                func=display_results,
                inputs=["linear_rmse", "linear_r2", "rf_rmse", "rf_r2", "xgb_rmse", "xgb_r2", "xgb_predictions"],
                outputs=None,
                name="display_results_node",
            ),
            node(
                func=save_predictions_to_text,
                inputs=["xgb_predictions", "params:output_path"],
                outputs=None,
                name="save_predictions_node",
            ),
        ]
    )
